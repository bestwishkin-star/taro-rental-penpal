...
import { fetchFavorites, fetchMyRentals, fetchUserProfile, getBrowseHistory, login } from '@/shared/api/services';
...
/** 个人中心页，管理屋檐记、收藏、历史和资料设置。 */
export default function ProfilePage() {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const { isLoggedIn, profile, profileStats, handleLoginSuccess, setProfile, setProfileStats } = useAuthStore();

  useDidShow(() => {
    setTabBarSelected(2);
    if (!isLoggedIn) return;

    void Promise.allSettled([fetchUserProfile(), fetchMyRentals(), fetchFavorites(), Promise.resolve(getBrowseHistory())]).then(
      ([profileResult, mineResult, favoriteResult, historyResult]) => {
        ...
        setProfileStats({
          ...
          favoriteCount: favoriteResult.status === 'fulfilled' ? favoriteResult.value.length : profileStats.favoriteCount,
    ...
    Taro.login({
      success: (res) => {
        ...
        login(res.code)
          ...
          .catch(() => {
            void Taro.showToast({ title: '登录失败，请稍后再试', icon: 'none', duration: 2000 });
          });
      },
      fail: () => {
        void Taro.showToast({ title: '登录失败', icon: 'none', duration: 2000 });
  ...
  return (
    <PageShell>
      <ProfileHeader
        ...
        stats={[
          { value: profileStats.publishCount, label: '我的屋檐记' },
      ...
      <View className="profile-menu">
        ...
        />
        <ProfileMenuItem icon={iconSettings} label="设置" desc="维护头像、昵称和居住偏好" onClick={navigateToSettings} />
      </View>
